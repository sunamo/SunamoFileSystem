...
foreach ($line in $lines) {
    if ($line -match '(.*?)\((\d+),(\d+)\): warning (CS\d+): (.*)') {
        ...
        if ($file.EndsWith('\FS.cs')) {
            $results += [PSCustomObject]@{Line=$lineNum; Col=$col; Code=$code; Msg=$msg}
        }
    }
}
$results | Sort-Object Line | ForEach-Object { Write-Output "$($_.Line)|$($_.Code)|$($_.Msg)" }
Write-Output ""
Write-Output "Total unique warnings: $($results.Count)"
